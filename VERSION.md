...
Changelog for End-to-End Pipeline, Gate CLI, Preprocessor Coverage Push, and Property-Based Leakage Tests
Version: 3.3.0 (vertical slice pipeline, Gate CLI Appendix C/D, coverage 56%→80%, 20 Hypothesis leakage tests)

### Major Themes Across All Changes

* **End-to-end vertical slice operational:** JavaFX UI → Python subprocess → preprocessing → backtesting → gate validation → JSON results displayed in desktop UI. Single command (`python -m srcPy.bridge.run_pipeline`) produces a complete, auditable `run_bundle_v1/` directory.
* **Gate CLI Appendix C/D v5.1 compliant:** `srcPy/cli/gate.py` validates all 5 required bundle files, enforces plan identity with expected/actual evidence, checks leakage invariants with purge gap verification, and returns structured `gate_result.json` with exit codes 0/1/2/3.
* **Property-based leakage detection:** 20 Hypothesis tests covering 5 leakage invariants (temporal ordering, sample disjointness, purge gap, fit-on-train-only, no look-ahead bias), exceeding the Phase 0 target of ≥5 tests.
* **Preprocessor coverage push:** 7 critical bug fixes + ~1,500 lines of new tests pushed `srcPy/preprocessor/*` from 56.1% → 79.9% line coverage. Branch coverage at 92% (target 70% exceeded).
* **Java integration hardened:** BacktestService with JSON extraction from mixed Python stdout, PythonRunner subprocess management, AtlantaFX theming, results card displaying validation status/Sharpe/drawdown/win rate/trade count.

---

### Detailed Changes

#### `srcPy/bridge/run_pipeline.py` (new)

* **Main orchestrator (~450 lines):** Assembles `run_bundle_v1/{timestamp}_{hash}/` directories with all Appendix C artifacts.
* **Pipeline flow:** Create bundle dir → write `plan.json` (IN_PROGRESS) → write `env_fingerprint.json` → preprocess (load OHLCV, compute returns/SMA/RSI) → backtest (SMA crossover signals → Sharpe, drawdown, win rate) → write manifests → invoke Gate CLI → update `plan.json` (COMPLETE) → emit JSON to stdout.
* **Exit codes per Appendix E:** `0`=SUCCESS, `1`=FAILED_GATES, `2`=INVALID_INPUT, `3`=ERROR.
* **Stdout contract:** Single JSON object with `status`, `bundle_path`, `message`, `artifacts` map. All logs to stderr.

#### `srcPy/cli/gate.py` (new)

* **Full bundle validation (42 tests):** Validates all 5 required files per Appendix C.2 (`plan.json`, `env_fingerprint.json`, `dataset_manifest.json`, `preprocessing_report.json`, `splits_manifest.json`) with schema version checks.
* **Plan identity gate:** Expected/actual evidence pattern per Appendix D.2 with `reason_code` (VALID, MISSING_PLAN_HASH, HASH_MISMATCH, UNKNOWN_SCHEMA_VERSION).
* **Leakage invariant enforcement:** Purge gap verification with timestamp parsing.
* **Exit codes per Appendix D.1:** `0`=all gates PASS, `1`=one or more FAIL, `2`=invalid input (missing bundle, malformed JSON, unknown schema version), `3`=internal error.
* **Output:** `gate_result.json` with `schema_version`, `overall_result`, `gates[]` array, `metadata`.

#### `srcPy/preprocessor/core.py` (new)

* **Core preprocessing functions (~140 lines):** `load_ohlcv()`, `add_returns()`, `add_sma()`, `add_rsi()`.
* **Coverage:** 93%.

#### `srcPy/backtest/engine.py` (new)

* **Backtesting engine (~180 lines):** `BacktestResult` dataclass, `add_signals()` (SMA crossover), `compute_metrics()` (total return, Sharpe ratio, max drawdown, win rate, trade count).

#### `srcPy/cli/preprocess.py` and `srcPy/cli/backtest.py` (new)

* **Standalone CLI tools** for preprocessing and backtesting individual files.

#### Bug Fixes — `srcPy/preprocessor/graph/` (7 files)

* **`factory.py`:** `SentimentLexicon` not imported + silent exception swallowing. Aliases "RSI", "SMA" never registered.
* **`planner.py`:** `self.metrics` / `self.weights` undefined — `_reoptimize()` crashed.
* **`api.py`:** Dead code, duplicate conversion logic. Cleaned up. Coverage: 40% → 83%.
* **`executor.py`:** `lru_cache` on instance method (broken). Replaced with dict cache. Coverage: → 55%.
* **`ops.py`:** `@cached_property` incompatible with subclass overrides. Changed to `@property` + manual cache. Coverage: → 90%.
* **`ops_custom.py`:** Method overrides didn't match base class. Updated to `_compute_requires/provides`. Coverage: 57% → 96%.
* **`expr.py`:** `frozen=True` dataclass with mutable parent. Removed frozen. Coverage: 45% → 93%.

#### Dead Code Removed

* **`expr_opt.py`:** Deleted (28 lines, 0% coverage). Duplicated logic already in `expr.py`.

#### Java Integration (modified)

* **`utils/PythonRunner.java`:** Full subprocess implementation for Python pipeline invocation.
* **`services/BacktestService.java`:** New service with JSON extraction from mixed Python stdout (logging mixed with JSON output resolved via Java-side extraction). Fixed dependency injection (static → instance-based PythonRunner).
* **`features/dashboard/DashboardController.java`:** Backtest integration + results display.
* **`resources/fxml/Dashboard.fxml`:** Backtest UI controls + results card (validation status, total return, Sharpe ratio, max drawdown, win rate, trade count).
* **`pom.xml`:** AtlantaFX dependency, Java 24, mainClass fix.
* **`MainApp.java`:** AtlantaFX theme initialization.

#### `tests/python/property/test_leakage_invariants.py` (new)

* **20 Hypothesis tests covering 5 leakage invariants:**
  * **Temporal ordering:** `max(train_timestamps) < min(test_timestamps)` for walk-forward splits.
  * **Sample disjointness:** `train_indices ∩ test_indices = ∅` across all fold types.
  * **Purge gap:** `test_start - train_end ≥ purge_window` enforced.
  * **Fit-on-train-only:** Scaler `.mean_`/`.std_` computed only from training data.
  * **No look-ahead bias:** Features at time `t` use only data ≤ `t`.

#### `tests/python/integration/test_vertical_slice.py` (new)

* **10 end-to-end integration tests:** Preprocess → backtest → gate validation → structured result (PASS or FAIL). Tests cover bundle production, JSON stdout parsing, exit code verification, and gate result schema compliance.

#### `tests/python/unit/preprocessor/` (4 new test files, ~1,500 lines)

* **`test_dsl_and_api.py`:** Targets `dsl.py` (0% → 80%) and `api.py` (40% → 83%).
* **`test_transforms_and_columns.py`:** Targets `transforms.py` (36% → 88%) and `columns.py` (31% → 86%).
* **`test_expr_ops_executor.py`:** Targets `expr.py` (45% → 93%), `ops_custom.py` (57% → 96%).
* **`test_executor_polars_coverage.py`:** Targets remaining gaps in `executor.py` and `backends/polars.py`.

#### `tests/fixtures/sample_spy.csv` (new)

* **Test data:** 61-row SPY OHLCV sample for pipeline testing.
...
|--------|--------|-------|--------|
| Line coverage (`srcPy/preprocessor/*`, excl GPU) | 56.1% | 79.9% | +23.8% |
| Branch coverage | 92% | 92% | ✅ Already exceeded 70% target |

**Files at 80%+:** `api.py` (83%), `core.py` (93%), `dsl.py` (80%), `expr.py` (93%), `graph.py` (94%), `ops.py` (90%), `ops_custom.py` (96%), `splits.py` (81%), `columns.py` (86%), `errors.py` (83%), `transforms.py` (88%)

**Remaining gap to 85% target (~125 lines):**
* `backends/polars.py`: 42% (~67 uncovered lines)
* `executor.py`: 55% (~60 uncovered lines)
...
| Suite | Tests | Status |
|-------|------:|--------|
| Gate CLI (`test_gate.py`) | 42 | ✅ Passing |
| Splits unit (`test_splits.py`) | 33 | ✅ Passing |
| Leakage invariants (`test_leakage_invariants.py`) | 20 | ✅ Passing |
| Gate core (`test_core.py`) | 19 | ✅ Passing |
| Backtest engine (`test_engine.py`) | 10 | ✅ Passing |
| Vertical slice integration (`test_vertical_slice.py`) | 10 | ✅ Passing |
| Splits property (`test_splits_properties.py`) | 7 | ✅ Passing |
| Preprocessor core (`test_core.py`) | 7 | ✅ Passing |
| Preprocessor coverage push (4 files) | ~1,500 lines | ✅ Passing |

**Total new/updated tests in 3.3.0:** 150+

---

### Behavioral Changes

* **Pipeline produces run bundles:** `run_pipeline.py` creates versioned `run_bundle_v1/{timestamp}_{hash}/` directories with deterministic naming.
* **Gate CLI invoked automatically:** Pipeline calls gate validation as step 7 and includes gate result in stdout JSON.
* **Java UI displays backtest results:** BacktestService parses Python stdout JSON and populates dashboard results card.
* **Bundle status lifecycle:** `plan.json` tracks `IN_PROGRESS` → `COMPLETE` or `FAILED` with timestamps.
* **Exit code semantics:** Pipeline and Gate CLI both use 0/1/2/3 scheme per Appendix D/E.

---

### Breaking Changes

* **`expr_opt.py` deleted:** Any imports from this module must be redirected to `expr.py`.
* **`factory.py` aliases:** "RSI" and "SMA" aliases now properly registered (code that relied on the broken silent failure will now see these aliases resolved).
* **`executor.py` caching:** `lru_cache` replaced with dict cache — behavior is equivalent but cache invalidation semantics differ.
* **`ops.py` properties:** `@cached_property` replaced with `@property` + manual cache — subclass overrides now work correctly.

---

### Files Created

* `srcPy/bridge/run_pipeline.py` — ~450 lines
* `srcPy/preprocessor/core.py` — ~140 lines
* `srcPy/backtest/engine.py` — ~180 lines
* `srcPy/cli/gate.py` — Gate CLI (42 tests)
* `srcPy/cli/preprocess.py` — Preprocessing CLI
* `srcPy/cli/backtest.py` — Backtesting CLI
* `marketmind_gate/gates/core.py` — Gate validation framework
* `tests/python/property/test_leakage_invariants.py` — 20 Hypothesis tests
* `tests/python/integration/test_vertical_slice.py` — 10 integration tests
* `tests/python/unit/preprocessor/test_dsl_and_api.py` — Coverage push
* `tests/python/unit/preprocessor/test_transforms_and_columns.py` — Coverage push
* `tests/python/unit/preprocessor/test_expr_ops_executor.py` — Coverage push
* `tests/python/unit/preprocessor/test_executor_polars_coverage.py` — Coverage push
* `tests/python/unit/preprocessor/test_core.py` — 7 tests
* `tests/python/unit/backtest/test_engine.py` — 10 tests
* `tests/python/unit/gate/test_core.py` — 19 tests
* `tests/python/unit/cli/test_gate.py` — 42 tests
* `tests/fixtures/sample_spy.csv` — 61-row test data

### Files Modified

* `srcPy/preprocessor/graph/factory.py` — Bug fix (import + exception swallowing)
* `srcPy/preprocessor/graph/planner.py` — Bug fix (undefined attributes)
* `srcPy/preprocessor/graph/api.py` — Bug fix (dead code removal)
* `srcPy/preprocessor/graph/executor.py` — Bug fix (lru_cache replacement)
* `srcPy/preprocessor/graph/ops.py` — Bug fix (cached_property replacement)
* `srcPy/preprocessor/graph/ops_custom.py` — Bug fix (method override alignment)
* `srcPy/preprocessor/graph/expr.py` — Bug fix (frozen dataclass removal)
* `src/main/java/**/utils/PythonRunner.java` — Subprocess implementation
* `src/main/java/**/services/BacktestService.java` — Pipeline invocation + JSON extraction
* `src/main/java/**/features/dashboard/DashboardController.java` — Backtest integration
* `src/main/resources/fxml/Dashboard.fxml` — UI controls + results card
* `pom.xml` — AtlantaFX, Java 24, mainClass
* `MainApp.java` — AtlantaFX theming

### Files Deleted

* `srcPy/preprocessor/graph/expr_opt.py` — 28 lines, 0% coverage, duplicated logic

---

### Phase 0 Checklist Items Completed (This Release)

* ✅ Run Bundle v1 emitted by pipeline (`run_bundle_v1/{timestamp}_{hash}/`)
* ✅ Gate CLI invoked automatically (42 tests, Appendix C/D v5.1 compliant)
* ✅ Java↔Python bridge stable (stdout JSON + exit codes per Appendix E)
* ✅ End-to-end UI demo (JavaFX → Python → backtest → gates → results card)
* ✅ Integration canary (10 vertical slice tests)
* ✅ ≥5 leakage property tests (20 Hypothesis tests, 5 invariants)

### Remaining Phase 0 Items

* 🚧 Preprocessor ≥85% line coverage (currently 79.9%, ~125 lines remaining)
* ❌ Baseline locked (CI == local coverage ≤0.5% drift)
* Record 2-minute demo video (interview proof point)
